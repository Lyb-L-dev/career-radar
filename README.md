...
- 内置 1188 家工信部公开名单企业候选库（其中福建 44 家），支持画像初筛、收藏/淘汰审批和人工核验官网后分批加入监控；候选库本身不访问网站、不消耗 LLM 配额。
...
├─ CANDIDATE_CATALOG.md       # 1188 家候选库来源、边界与背调流程
...
前端项目位于同级目录 `E:\AIProjects\work\career-radar-web`。首次安装或前端代码变化后构建一次：
...
开发前端时使用两个 PowerShell 窗口：后端运行上面的 `serve` 命令；前端目录运行 `npm run dev`，再访问 `http://127.0.0.1:7100`。Vite 会把 `/api` 代理到 8000 端口。

安全说明：`serve` 会拒绝绑定公网地址，表单也会拒绝 `localhost`、私网和保留 IP，避免把本地管理端变成内网请求代理。Web 不返回 DeepSeek/SMTP 密钥；密钥仍只从 `.env` 或系统环境变量读取。若以后部署到服务器并允许远程访问，必须另外配置反向代理认证和 HTTPS。
...
`config.yaml` 当前有 30 家正式监控公司/官方公告源，其中 22 家启用。另有 1188 家官方名单候选企业保存在 `data/company_candidates.json`，不会自动参加扫描；请先按 [候选库说明](CANDIDATE_CATALOG.md) 完成人工背调和官网核验，再从管理端分批加入。现有正式公司的筛选依据、官方来源和局限见 [COMPANY_SELECTION.md](COMPANY_SELECTION.md)。每家公司最少需要 `name` 和 `url`：
